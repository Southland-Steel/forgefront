...
.status-dot.checking { background: #9ca3af; animation: pulse 1.2s ease-in-out infinite; }
@keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.4} }
.status-cell { display: flex; align-items: center; gap: 8px; min-width: 140px; }
...
.proto-tcp   { background: #ede9fe; color: #6d28d9; }
...
<div class="container-fluid px-4 pt-3">
    <div class="page-header d-flex justify-content-between align-items-center mb-3">
        <h4 class="page-title"><i class="fas fa-server me-2" style="background: linear-gradient(to bottom, #4ade80 0%, #16a34a 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;"></i>Server Board</h4>
    ...
    <div class="card">
        <div class="card-body p-0">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-hover mb-0" id="serverTable">
                    <thead>
                        <tr>
                            <th class="ps-3" style="width:160px">Status</th>
                            <th>Name</th>
                            <th>Host</th>
                        ...
                        <tr id="row-<?= $s['server_id'] ?>">
                            <td class="ps-3">
                                <div class="status-cell">
                                    <span class="status-dot checking" id="dot-<?= $s['server_id'] ?>"></span>
                                    <span id="status-text-<?= $s['server_id'] ?>" class="small text-muted">Checking...</span>
                                </div>
                            </td>
                            <td class="fw-semibold"><?= htmlspecialchars($s['name']) ?></td>
                            <td class="host-mono"><?= htmlspecialchars($s['host']) ?>:<?= $s['port'] ?></td>
                            <td>
                                <span class="proto-badge proto-<?= $s['protocol'] ?>"><?= strtoupper($s['protocol']) ?></span>
                            </td>
                            <td class="small text-muted"><?= htmlspecialchars($s['description'] ?? '') ?></td>
                            <td class="pe-3 text-end">
                                <button class="btn btn-sm btn-outline-secondary py-0 px-2 me-1" type="button"
                                    onclick="openEditModal(<?= htmlspecialchars(json_encode($s)) ?>)">
                                    <i class="fas fa-pencil"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger py-0 px-2" type="button"
                                    onclick="confirmDelete(<?= (int)$s['server_id'] ?>, <?= htmlspecialchars(json_encode($s['name'])) ?>)">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-6">
                        ...
                        <input type="number" class="form-control" id="serverPort" min="1" max="65535" placeholder="80">
                    </div>
                </div>
                <div class="mb-3">
...
function setStatus(id, data) {
    ...
    let label = '';
    if (data.status === 'online')  label = `Online${data.ms != null ? ` <span class="ms-badge">${data.ms}ms</span>` : ''}`;
    if (data.status === 'offline') label = `<span class="text-danger">Offline</span>`;
    if (data.status === 'warning') label = `<span class="text-warning">${data.detail}</span>`;
    text.innerHTML = label || data.detail || '';
}
...
if (serverIds.length) {
    checkAll();
    refreshTimer = setInterval(checkAll, 30000);
}
</script>
